...
func (pq *PlaylistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Playlist, error) {
	...
	if query := pq.withSongs; query != nil {
		if err := pq.loadSongs(ctx, query, nodes,
			func(n *Playlist) { n.Edges.Songs = []*PlaylistSong{} },
			func(n *Playlist, e *PlaylistSong) { n.Edges.Songs = append(n.Edges.Songs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlaylistQuery) loadSongs(ctx context.Context, query *PlaylistSongQuery, nodes []*Playlist, init func(*Playlist), assign func(*Playlist, *PlaylistSong)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "playlist_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pgb *PlaylistGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (ps *PlaylistSelect) sqlScan(ctx context.Context, v any) error {
	rows := &sql.Rows{}
