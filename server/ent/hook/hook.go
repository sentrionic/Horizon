...
// The PlaylistSongFunc type is an adapter to allow the use of ordinary
// function as PlaylistSong mutator.
type PlaylistSongFunc func(context.Context, *ent.PlaylistSongMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PlaylistSongFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.PlaylistSongMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PlaylistSongMutation", m)
	}
	return f(ctx, mv)
}
