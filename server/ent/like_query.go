...
func (lq *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	...
	if query := lq.withSong; query != nil {
		if err := lq.loadSong(ctx, query, nodes, nil,
			func(n *Like, e *Song) { n.Edges.Song = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LikeQuery) loadSong(ctx context.Context, query *SongQuery, nodes []*Like, init func(*Like), assign func(*Like, *Song)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Like)
	for i := range nodes {
		fk := nodes[i].SongID
