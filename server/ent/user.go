...
type User struct {
	...
	Role horizon.Role `json:"role,omitempty"`
...
func (e UserEdges) SongsOrErr() ([]*Song, error) {
	if e.loadedTypes[0] {
		return e.Songs, nil
	}
...
func (*User) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldUsername, user.FieldEmail, user.FieldPassword, user.FieldImage, user.FieldRole:
	...
	for i := range columns {
		...
		case user.FieldRole:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				u.Role = horizon.Role(value.String)
			}
		}
	}
	return nil
...
// QuerySongs queries the "songs" edge of the User entity.
func (u *User) QuerySongs() *SongQuery {
	return (&UserClient{config: u.config}).QuerySongs(u)
}
